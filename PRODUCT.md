...
34. 作为调用方，我希望 EgressKit 在 VLESS 建连失败时尝试其他健康节点，从而提高建立隧道的成功率。
...
38. 作为运维者，我希望节点的连接失败、DNS 失败、TLS/Reality 握手失败和超时影响节点健康，从而及时避开不可用出口。
...
42. 作为运维者，我希望节点经历 warming、healthy、degraded、cooldown、draining、disabled 和 removed 状态，从而理解节点为什么可用或不可用。
43. 作为运维者，我希望手动禁用节点不会被健康检查自动恢复，从而保留人工控制权。
44. 作为运维者，我希望冷却节点在探活成功后自动恢复，从而无需手工恢复临时故障节点。
45. 作为运维者，我希望健康检查真正经过对应的 Mihomo listener，从而同时验证 VLESS、TLS、Reality 和目标访问能力。
46. 作为运维者，我希望可以配置多个健康检查 URL 和成功阈值，从而避免单个检测站故障误判所有节点。
47. 作为运维者，我希望健康检查限制并发，从而避免订阅节点很多时产生探活风暴。
...
- 节点状态包括 warming、healthy、degraded、cooldown、draining、disabled 和 removed。
...
- 节点评分默认综合 EWMA 延迟、成功率、连续失败、活动连接数、手动权重和冷却状态。
- 默认从高分候选中进行加权选择，避免所有流量长期集中在单一最低延迟节点。

### 健康检查

- 主动健康检查必须通过节点对应的内部 Mihomo listener 发出，不能只测试远端 TCP 端口。
- 支持多个可配置健康 URL 和成功阈值；默认任一目标成功即可证明基础可用。
- 健康检查限制并发并加入调度抖动，避免刷新后同时探测全部节点。
- 第一次传输失败降低权重，连续失败进入 degraded，再达到阈值后进入 cooldown；冷却后通过 warming 探活恢复。
- 初始冷却建议为 60 秒，重复失败指数增加，最大冷却 15 分钟。
...
- 时间相关逻辑使用可控制的时钟 seam，覆盖 TTL、空闲过期、退避、冷却和 operation 中断恢复。
...
- 验证手动 disabled 不会被健康检查自动恢复。
- 验证 cooldown 到期后必须先探活再变为 healthy。
...
6. 订阅异常下降保护、draining、熔断、冷却和回滚均通过行为测试。

...
节点导入后还要经过健康检查和出口 IP 识别。所有节点都会进入验证队列，出口识别单次最多并发 5 个；节点较多时需要等待几分钟。
...
flowchart LR
    ...
    health["探活与出口识别"] --> registry
    registry --> scheduler
    registry <--> sqlite[("SQLite")]
```

多个节点可能共用一个公网 IP，所以 EgressKit 按经过验证的出口 IP 调度，而不是按节点数量轮换。`rotate` 先选择最长未使用的 IP 组，再从组内选择健康节点；sticky、strict 和指定节点模式也都绑定到出口 IP。

### 代理生命周期

```mermaid
stateDiagram-v2
    [*] --> 验证中: 导入节点 / generation 变化

    验证中 --> 待识别出口: 探活成功，但尚无出口 IP
    待识别出口 --> 验证中: 下个探活周期重试
    验证中 --> 可调度: 探活成功且获得出口 IP
    待识别出口 --> 可调度: 获得出口 IP
    ...
    可调度 --> 排空中: 订阅替换或移除旧 generation
    排空中 --> 已移除: 已有连接归零
    已移除 --> [*]
```

“探活成功”和“可调度”不是同一个条件：代理必须同时健康并拥有已验证的出口 IP，才会进入调度池。连续失败默认先进入 `degraded`，再进入指数退避的 `cooldown`；冷却到期后回到 `warming` 并重新探活。人工停用不会被自动探活覆盖。订阅更新产生新 generation 时，新代理重新验证，旧代理停止接收新连接；已有连接结束后，旧 listener 才会被移除并隔离端口。
...
- `/ready` 返回 503：尚无节点同时通过健康检查和出口 IP 验证。
